...
class PrintDashboardNotifier extends StateNotifier<PrintDashboardState> {
  ...
  Future<void> fetchJobs({
    ...
    try {
      ...
      state = state.copyWith(isLoading: false, allJobs: sortedList);
      _applyLocalFilterAndPagination();
    } catch (e) {
      state =
  ...
  Future<void> executePrint(String mode, List<Map<String, dynamic>> dataToPrint,
      {String username = '', required bool isPdf}) async {
    ...
    state = state.copyWith(
        isPrinting: true,
        statusMessage: '⏳ กำลังประมวลผลข้อมูลและส่งเข้าเครื่องพิมพ์...');

    try {
      PrintStrategy strategy;
      if (isPdf) {
        strategy = (mode == 'job' || mode == 'order')
            ? PdfJobPrintStrategy()
            : PdfFuelPrintStrategy(username);
      } else {
        strategy = (mode == 'job' || mode == 'order')
            ? RawJobPrintStrategy()
            : RawFuelPrintStrategy(username);
      }
